        ...
        protected void ReadChunked(Stream stream)
        {
            ...
            using (var output = new BufferPoolMemoryStream())
            {
                ...
                using (var _ = new PooledBuffer(buffer))
                {
                    ...
                    Decompression.IDecompressor decompressor = baseRequest.UseStreaming ? GetDecompressor(encoding) : null;
...
#region Stream Decoding
        ...
        internal Decompression.IDecompressor GetDecompressor(string encoding)
        {
            if (encoding == null)
                return null;
            ...
            switch (encoding.ToLowerInvariant())
            {
                ...
                case "gzip": return new Decompression.GZipDecompressor(256);

#if NET_STANDARD_2_1 || UNITY_2021_2_OR_NEWER
                case "br": return new Decompression.BrotliDecompressor(256);
#endif
                default:
                    HTTPManager.Logger.Warning("HTTPResponse", "GetDecompressor - unsupported encoding: " + encoding, this.Context);
                    break;
            }
...
#if NET_STANDARD_2_1 || UNITY_2021_2_OR_NEWER
                    case "br": decoderStream = new System.IO.Compression.BrotliStream(streamToDecode, System.IO.Compression.CompressionMode.Decompress, true); break;
#endif
#endif
                    //identity, utf-8, etc.
                    default:
