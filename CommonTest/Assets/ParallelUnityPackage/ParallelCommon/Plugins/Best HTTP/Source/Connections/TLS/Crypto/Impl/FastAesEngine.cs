        ...
        public unsafe int ProcessBlock(ReadOnlySpan<byte> input, Span<byte> output)
        {
            ...
            if (forEncryption)
            {
                ...
                fixed (byte* poutput = output)
                {
                    ...
                    puoutput[0] = C0; //Pack.UInt32_To_LE(C0, output);
                    puoutput[1] = C1; //Pack.UInt32_To_LE(C1, output[4..]);
                    puoutput[2] = C2; //Pack.UInt32_To_LE(C2, output[8..]);
                    puoutput[3] = C3; //Pack.UInt32_To_LE(C3, output[12..]);
                }
            }
            else
            {
