        ...
        public unsafe int ProcessBytes(ReadOnlySpan<byte> input, Span<byte> output)
        {
            ...
            else
            {
                ...
                while (input.Length >= inLimit2)
                {
                    ...
                    fixed (byte* pS = S)
                    fixed (byte* poutput = outputSample)
                    fixed (byte* pctrBlock = ctrBlock)
                    {
                        uint* puint = (uint*)pinput;
                        uint* puS = (uint*)pS;
                        uint* puoutput = (uint*)poutput;
                        uint* puctrBlock = (uint*)pctrBlock;

                        puS[0] ^= puint[0];
                        puoutput[0] = puint[0] ^ puctrBlock[0];
                        puS[1] ^= puint[1];
                        puoutput[1] = puint[1] ^ puctrBlock[1];
                        puS[2] ^= puint[2];
                        puoutput[2] = puint[2] ^ puctrBlock[2];
                        puS[3] ^= puint[3];
                        puoutput[3] = puint[3] ^ puctrBlock[3];
                    }
