...
namespace BestHTTP.Connections.TLS.Crypto.Impl
{
    internal static class FastAesEngineHelper
    {
        public unsafe static void EncryptBlock(byte[] input, int inOff, byte[] output, int outOff, uint[][] KW, int ROUNDS, uint[] T0, byte[] S, byte[] s)
        {
            ...
            uint C2 = 0;
            uint C3 = 0;
            fixed (byte* pinput = &input[inOff])
            {
                uint* puinput = (uint*)pinput;

                C0 = puinput[0];//Pack.LE_To_UInt32(input, inOff + 0);
                C1 = puinput[1];//Pack.LE_To_UInt32(input, inOff + 4);
                C2 = puinput[2];//Pack.LE_To_UInt32(input, inOff + 8);
                C3 = puinput[3];//PPack.LE_To_UInt32(input, inOff + 12);
            }
            ...
            fixed (byte* poutput = &output[outOff])
            {
                uint* puoutput = (uint*)poutput;

                puoutput[0] = C0; //Pack.UInt32_To_LE(C0, output, outOff + 0);
                puoutput[1] = C1; //Pack.UInt32_To_LE(C1, output, outOff + 4);
                puoutput[2] = C2; //Pack.UInt32_To_LE(C2, output, outOff + 8);
                puoutput[3] = C3; //Pack.UInt32_To_LE(C3, output, outOff + 12);
            }
        }
    }
}
