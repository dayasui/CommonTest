...
public static class BuildManager {
    ...
    private static void AddressableBuildCore(string profileName, UnityAction<AddressableAssetBuildResult> onComplete) {
        var settings = GetAddressableAssetSettings();

        var defProfileID = settings.activeProfileId;
        var defPlayModeIndex = settings.ActivePlayModeDataBuilderIndex;
        ...
        BuildScript.buildCompleted = (result) => {
            Debug.Log("Addressable buildComplete!");
            settings.activeProfileId = defProfileID;
            settings.ActivePlayModeDataBuilderIndex = defPlayModeIndex;
            onComplete(result);
        };
        // ビルド
    ...
    private static AddressableAssetGroup GetGroup(string groupName) {
        return GetAddressableAssetSettings()?.FindGroup(groupName);
    }

    private static List<AddressableAssetGroup> GetGroups() {
        return GetAddressableAssetSettings()?.groups;
    }
