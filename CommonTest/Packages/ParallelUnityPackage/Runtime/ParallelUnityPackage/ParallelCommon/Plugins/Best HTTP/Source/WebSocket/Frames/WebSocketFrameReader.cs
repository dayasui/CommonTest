...
#region Properties
        ...
        internal unsafe void Read(Stream stream)
        {
            ...
            try
            {
                ...
                if (HasMask)
                {
                    fixed (byte* pData = buffer, pmask = mask)
                    {
...
#region Public Functions
        ...
        {
            return string.Format("[{0} Header: {1:X2}, IsFinal: {2}, HasMask: {3}, Data: {4}]", this.Type.ToString(), this.Header, this.IsFinal, this.HasMask, this.Data);
        }
