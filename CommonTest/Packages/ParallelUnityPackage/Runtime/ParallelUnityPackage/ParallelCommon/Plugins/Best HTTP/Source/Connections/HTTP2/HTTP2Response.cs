        ...
        internal void AddData(Stream stream)
        {
            if (this.HasContentEncoding)
            {
                ...
                switch (this.contentEncoding)
                {
                    ...
                    case "gzip": decoderStream = new Decompression.Zlib.GZipStream(stream, Decompression.Zlib.CompressionMode.Decompress); break;
                    case "deflate": decoderStream = new Decompression.Zlib.DeflateStream(stream, Decompression.Zlib.CompressionMode.Decompress); break;
#if NET_STANDARD_2_1 || UNITY_2021_2_OR_NEWER
                    case "br": decoderStream = new System.IO.Compression.BrotliStream(stream, System.IO.Compression.CompressionMode.Decompress, true); break;
#endif
#endif
                    //identity, utf-8, etc.
                    default:
                        // Do not copy from one stream to an other, just return with the raw bytes
                        base.Data = new byte[stream.Length];// BufferPool.Get(stream.Length, false);
        ...
        internal void ProcessData(byte[] payload, int payloadLength)
        {
            ...
            if (this.HasContentEncoding)
            {
                if (this.decompressor == null)
                    this.decompressor = GetDecompressor(this.contentEncoding);
